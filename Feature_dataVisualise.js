...
function footprintPlot(latitude, longitude, radius) {
  //creating an array of angles from 0-2pi with a steo size of pi/50 to define the distance between points around the circle 
    ...
    latitudeCirc.push(latitudeCirc[0]);  //Push the 0th element to close the circle
    longitudeCirc.push(longitudeCirc[0]); //Push the 0th element to close the circle

    return [latitudeCirc, longitudeCirc]; // Output arguments will be in the form of two arrays, one fore each latitude value and one for each longitude value 
    }
